...
import { StyleSheet, Text as RNText, View as RNView, Pressable as RNPressable, TextInput as RNTextInput, type TextProps, type ViewProps, type PressableProps, type TextInputProps, ViewStyle, Platform } from 'react-native'
...
export type GetThemePlaceholderColorProps = {
  theme: ThemeType
  placeholderColor: 'input2Placeholder' | 'grey1_light3'
}
...
export function Pressable({ background = null, hoverBackground = null, border = null, style, className, ...props }: CustomPressableProps) {
  ...
  const handlHoverIn = () => hoverBackground && setIsHover(true)
  const handlHoverOut = () => setIsHover(false)
  ...
  // Only pass hover props on web — this prevents the crash on mobile
  const hoverProps = Platform.OS === 'web' ? { onHoverIn: handlHoverIn, onHoverOut: handlHoverOut } : {}

  return <RNPressable {...props} {...hoverProps} className={clsx('', className)} style={typeof style === 'function' ? style : [{}, currentBackground ? { backgroundColor: currentBackground } : null, currentBorder, style]} />
}
...
export function GradientPressable({ type, style, combinedStyle, gradientStyle, containerClassname, gradientClassname, combinedClassname, className, isPressable = true, ...props }: CustomGradientPressableProps) {
  const { theme } = useTheme()
